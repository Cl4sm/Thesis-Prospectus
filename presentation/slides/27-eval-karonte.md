...
window.addEventListener("load", function() {
  const metrics = [
    {label: "Binaries Analyzed", satc: 131, mango: 3599, mult: "22x"},
    {label: "Alerted Binaries", satc: 52, mango: 174, mult: "3.3x"},
    {label: "Total Alerts", satc: 144, mango: 2310, mult: "16x"}
  ];

  const margin = {top: 25, right: 110, bottom: 20, left: 190};
  ...
  const svg = d3.select("#chart-slide18").append("svg")
    .attr("viewBox", "0 0 950 260").attr("width", "100%")
    .append("g").attr("transform", `translate(${margin.left},${margin.top})`);

  const globalMax = Math.max(...metrics.map(m => Math.max(m.satc, m.mango)));
  const scale = d3.scaleLinear().domain([0, globalMax]).range([0, width]);
  ...
  metrics.forEach((m, i) => {
    ...
    svg.append("text").attr("x", -10).attr("y", gy + barH)
      .attr("text-anchor", "end").style("font-size", "17px").style("font-weight", "600").style("fill", "#333")
      .text(m.label);
    ...
    svg.append("text").attr("x", scale(m.satc) + 8).attr("y", gy + barH - 5)
      .style("font-size", "16px").style("fill", "#666").text(m.satc.toLocaleString());
    ...
    svg.append("text").attr("x", scale(m.mango) + 8).attr("y", gy + barH*2 + 3)
      .style("font-size", "16px").style("font-weight", "700").style("fill", "#8C1D40")
      .text(m.mango.toLocaleString());

    // Multiplier badge
    svg.append("text").attr("x", width + 50).attr("y", gy + barH + 10)
      .attr("text-anchor", "middle")
      .style("font-size", "22px").style("font-weight", "800").style("fill", "#8C1D40")
      .text(m.mult);

    // Tool labels (only on first group)
    if (i === 0) {
      svg.append("text").attr("x", scale(m.satc) + 55).attr("y", gy + barH - 5)
        .style("font-size", "13px").style("fill", "#999").text("SaTC");
      svg.append("text").attr("x", scale(m.mango) + 65).attr("y", gy + barH*2 + 3)
        .style("font-size", "13px").style("fill", "#8C1D40").text("Mango");
    }
  });
});
</script>
