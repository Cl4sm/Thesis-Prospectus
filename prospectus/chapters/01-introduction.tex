...
Software vulnerabilities are a primary enabler of this damage, providing footholds through which attackers breach systems,
...
Vulnerability researchers generate evidence of vulnerabilities not through any single technique, but through a combination of approaches~\cite{votipka2018hackers}.
...
Tools like Karonte~\cite{karonte}, SaTC~\cite{SaTC}, DTaint~\cite{cheng2018dtaint}, and Saluki~\cite{gotovchits2018saluki} all
adopt this approach.
...
Integer conversion, truncation, or sanitization are just a few of the factors that can cause a path to be unexploitable.
Tools that track only tainted values without encoding the full transformation history produce false positives, flagging paths where the value has been sanitized beyond exploitation \(over tainting\).
...
network input~\cite{SaTC, karonte}.
However, this filtering discards the majority of the firmware.
On the same dataset, Karonte and SaTC analyze a maximum of 131 of 3,599 binaries, leaving vulnerabilities in the remaining binaries unexamined.
...
This multi-technique coordination, the understanding of how to sequence independent analyses so that their findings reinforce one another, is
difficult to encode in an automated system without human intervention.
...
DARPA's AI Cyber Challenge (AIxCC)~\cite{aixcc}, a \$29.5 million competition launched in 2024, challenged
...
In Chapter~\ref{ch:artiphishell}, I present Artiphishell, a fully autonomous CRS designed and led by team Shellphish for the AI Cyber Challenge.
...
\section{Symbolic Execution for Path Feasibility in Firmware}
...
reachable under conditions that trigger a bug.
The data flow may exist, and the value at the sink may appear exploitable, but the specific sequence of branches required to reach that sink
can be unsatisfiable given the program's actual constraints.
...
This path feasibility reasoning by humans is expensive, time-consuming, and requires domain expertise~\cite{votipka2019observational}.

Symbolic execution~\cite{king1976symbolic} automates path-based reasoning by treating program inputs as symbolic variables and collecting path constraints that,
